...
class RobotPlayer:
    def __init__(self):
        ...
        self.robot_gestures['lizard'] = [0.189, 0.0185, 0.06] #between rock and paper
        self.robot_gestures['spock'] = [0.189, 0.2035, 0.06] #between paper and scissors
        ...
        rospy.Subscriber("hand_gesture", String, self.gesture_callback, queue_size=1)
        rospy.sleep(1)
    ...
    def gesture_callback(self, data):
        # *****************************************
        # ************ To be done here ************
        cur_gesture = data.data
        print(cur_gesture, time.time())
        if cur_gesture == 'rock':
            print("See rock, To paper")
            self.goal = self.robot_gestures['paper']
        elif cur_gesture == 'paper':
            print("See paper, To scissors")
            self.goal = self.robot_gestures['scissors']
        elif cur_gesture == 'scissors':
            print("See scissors, To rock")
            self.goal = self.robot_gestures['rock']
        elif cur_gesture == 'lizard':
            print("See Lizard, To Rock")
            self.goal = self.robot_gestures['rock']
        elif cur_gesture == 'spock':
            print("See spock, To lizard")
            self.goal = self.robot_gestures['lizard']
        self.move_to_goal()
        # *****************************************
